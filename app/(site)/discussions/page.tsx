...
import { getAllDiscussions, getHeroPhoto } from "@/lib/sanity.queries";
...
export default async function DiscussionsPage() {
  const [discussions, heroPhoto] = await Promise.all([
    getAllDiscussions(),
    getHeroPhoto(3),
  ]);
      ...
      <PageHero photo={heroPhoto}>
        <div className="mx-auto w-full max-w-2xl px-6">
          ...
          <p className="mt-2 text-star-500">
            One topic a week — planning, processing, gear, whatever&apos;s worth
            talking through. Comment with a GitHub account.
          </p>
        </div>
      </PageHero>
          ...
          <ul className="mt-10 divide-y divide-void-700">
            {discussions.map((d) => (
              <li key={d._id} className="py-5">
                <Link
                  href={`/discussions/${d.slug.current}`}
                ...
                >
                  ...
                  {d.topic && (
                    <p className="mt-1 text-sm text-star-500">{d.topic}</p>
                  )}
                </Link>
              </li>
            ))}
