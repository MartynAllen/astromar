...
export const metadata: Metadata = {
  ...
  description: "Starting points for astrophotography, from the smart-telescope side of the hobby.",
};

export default async function GuidePage() {
  const articles = await getAllGuideArticles();
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-14">
      ...
      <p className="mt-2 text-star-500">
        ...
        manual mounts or PixInsight.
      </p>

      {sections.length === 0 ? (
        <p className="mt-16 text-center text-star-500">No guide articles published yet.</p>
      ) : (
                ...
                {articles
                  ...
                  .map((article) => (
                    <li key={article._id} className="py-4">
                      <Link href={`/guide/${article.slug.current}`} className="group block">
                        ...
                        {article.summary && (
                          <p className="mt-1 text-sm text-star-500">{article.summary}</p>
                        )}
                      </Link>
                    </li>
                  ))}
